...
public class POO2223 {
	...
	private static void alinea2(PrintStream out) {
		...
		*/
		...
		try (Scanner sc = new Scanner(new File("POO2223/bookings.txt"))){
			int count = 0;
			...
			while(sc.hasNextLine()){
				...
				String[] words = linhas.split("\\;");

				int contribuinte = Integer.parseInt(words[0]);
				LocalDate startDate = LocalDate.parse(words[1]);
				int duracao = Integer.parseInt(words[2]);
				SpaceType tipo = SpaceType.getSpaceType(words[3]);
				int [] minDim = {Integer.parseInt(words[4].split("x",2)[0]),Integer.parseInt(words[4].split("x",2)[1])};
			
				Client c = campingService.getClient(contribuinte);
				List<CampingSpace> lista = campingService.findAvailableCampingSpaces(tipo, startDate, duracao, minDim);
				if (lista != null){
					for (CampingSpace campingSpace : lista) {
						campingService.bookCampingSpace(c, campingSpace, startDate,duracao);
					}
				}
