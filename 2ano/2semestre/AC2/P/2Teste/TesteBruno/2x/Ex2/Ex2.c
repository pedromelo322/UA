...
void send2display(int value){
    ...
    static const char setsegDecoder[] = {0x3F, 0x06, 0x5B, 0x4F,
    0x66, 0x6D, 0x7D, 0x07,
    0x7F, 0x6F, 0x77, 0x7C,
    0x39, 0x5E, 0x79, 0x71};
    ...
    if (flag == 0){
        ...
        LATDbits.LATD6 = 1;
        LATDbits.LATD5 = 0;
        LATB = (LATB & 0x80FF) | (setsegDecoder[value>>4] << 8);

    }else if(flag == 1){

        LATDbits.LATD6 = 0;
        LATDbits.LATD5 = 1;
        LATB = (LATB & 0x80FF) | (setsegDecoder[value&0x0f] << 8);
...
int main(void)
{
    ...
    T5CONbits.TCKPS = 4; // 1:16 prescaler (i.e. fout_presc = 25 Hz)
