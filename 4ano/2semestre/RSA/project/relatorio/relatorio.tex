...
\paragraph{Nota sobre os dois planos Zenoh.} Para simplicidade, a Figura~\ref{fig:arch}
representa um único bloco ``Zenoh Router''. Na realidade existem \emph{dois} planos de
comunicação distintos: (i)~o tráfego V2X (CAM/MCM) de cada veículo flui pelo broker
Zenoh do seu \emph{próprio} container Vanetza-NAP (\texttt{tcp/<ip>:7447}), simulando o
meio rádio local; (ii)~as mensagens de controlo da demonstração (cenário, \emph{ready},
\emph{done}) passam pelo router central \texttt{192.168.98.5:7446}. Assim, nenhum
veículo comunica com outro por um canal partilhado privilegiado --- toda a coordenação
...
juntam-se três nós de infraestrutura: o router Zenoh, o \texttt{bridge.py} e a
dashboard. Toda a topologia assenta na rede Docker \texttt{vanetzalan0}.
...
MCM transporta no \texttt{basicContainer} os campos \texttt{mcmType}, \texttt{itssRole}
e \texttt{manoeuvreId} que, em conjunto, identificam o subtipo --- nomeadamente, o
intervalo do \texttt{manoeuvreId} distingue um \textsc{merge\_grant} (0--127) de um
\textsc{slowdown\_grant} (128--255), que de outra forma seriam indistinguíveis.
...
\begin{table}[H]
  ...
  \texttt{itssRole}: 1 = \emph{coordinatingItss}, 3 = \emph{targetVehicle}.}
  \label{tab:mensagens}
\end{table}
