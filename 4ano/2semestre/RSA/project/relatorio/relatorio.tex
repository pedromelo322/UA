...
Python ---, onde vive toda a negociação. Segue uma ordem \emph{top-down}: começa por uma
vista geral do código e por uma leitura da negociação de ponta a ponta
(Secção~\ref{sec:visao-geral}); desce depois ao protocolo e às suas máquinas de estados
(Secção~\ref{sec:protocolo}); formaliza os fundamentos geométricos e cinemáticos que
sustentam os cálculos (Secção~\ref{sec:fundamentos}); descreve o transporte de mensagens e
a consciência situacional (Secção~\ref{sec:mensagens}); e termina na orquestração e
visualização (Secção~\ref{sec:orquestracao}).

\subsection{Visão geral e a negociação num relance}
...
\paragraph{A negociação num relance.} Antes de descer aos detalhes, a Figura~\ref{fig:seq}
dá a vista de conjunto do protocolo num cenário concreto: o MC funde-se na via onde
circulam A, B e C (A à frente, C atrás), estando apenas \textbf{B em conflito}. A sequência
decorre como se segue (os subtipos de MCM estão definidos na Tabela~\ref{tab:mensagens}):
...
Esta secção formaliza as primitivas de \texttt{geo.py} que sustentam os cálculos do
protocolo (Secção~\ref{sec:protocolo}): traduzem as posições GPS das CAMs em quantidades
operacionais e modelam a dinâmica de travagem. Todas as estradas são segmentos de reta
definidos no cenário por \texttt{start} e \texttt{end}.
...
(azimute referenciado a Norte). Esta projeção é usada pelo veículo da via principal para
reposicionar a zona de conflito na sua estrada (Secção~\ref{sec:lado-estrada}).
...
Esta secção descreve como as mensagens são construídas, transportadas e recebidas, e como
delas emerge a consciência situacional que alimenta todo o protocolo.
...
(Tabela~\ref{tab:mensagens}). O que vem a seguir distingue um \emph{pedido} de uma
\emph{resposta}:
...
Em esquema, um \textsc{merge\_request} do MC (\emph{station}~10) que pede a A
(\emph{station}~11) para abrandar para $15$\,m/s tem a forma:

\begin{lstlisting}[language=,basicstyle=\ttfamily\scriptsize]
basicContainer:    { mcmType=1, itssRole=1, manoeuvreId=7, lat, lon }   # pedido, MC (coordenador)
mcmContainer.vehicleManoeuvreContainer:
    vehicleCurrentStateContainer: { speed, heading, length }            # estado do MC
    targetRoadResource: zona de conflito = offsets(inicio, fim) + comprimento + janela
    manoeuvreAdvice: [ { executantID=11, advisedSpeed=15 m/s } ]        # destinatario = A
\end{lstlisting}

\textsc{merge\_grant} e \textsc{slowdown\_grant} partilham \texttt{mcmType=2} e
\texttt{itssRole=3}; distinguem-se apenas pelo intervalo do \texttt{manoeuvreId} (1 byte):
\textbf{0--127} para o primeiro e \textbf{128--255} para o segundo.
