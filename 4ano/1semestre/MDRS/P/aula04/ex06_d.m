...
b = 1e-6;        % Taxa de Erro de Bit (BER)
...
for it = 1:N
    ...
    [PL_values(it), APD_values(it), MPD_values(it), TT_values(it)] = Simulator2(lambda, C, f, P, b);
...
% Packet Loss (PL) - Praticamente não muda. O PL continua dominado pelo buffer overflow (f=2 KBytes é muito pequeno). A perda adicional por BER (0.49%, conforme 6.b) é insignificante perto da perda por overflow (≈44%).
% Av. Packet Delay (APD) - Permanece inalterado. Um buffer pequeno (2 KBytes) faz com que a maioria dos pacotes seja descartada imediatamente. Os poucos que esperam têm atrasos baixos, pois a fila não acumula. O BER não afeta o atraso.
% Throughput (TT) - Permanece baixo, limitado pela taxa de pacotes que conseguem entrar na fila (alta perda). A perda adicional por BER (0.49%) reduz marginalmente o TT.