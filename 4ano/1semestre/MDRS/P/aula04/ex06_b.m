...
% Packet Loss (PL) - O PL aumenta drasticamente. Como o BER é baixo (10−6) e a fila é grande (1 MByte), a maioria das perdas é agora devido a erros de transmissão, não a buffer overflow.
% Av. Packet Delay (APD) - O atraso de fila/sistema é dominado pela carga e capacidade, não pelo BER. O BER apenas determina se o pacote é descartado ou não após a transmissão.
% Throughput (TT) - O throughput diminui porque mais pacotes são perdidos (descartados no destino) e, portanto, menos bytes úteis são contabilizados no total de bytes transmitidos com sucesso.